...
import { addModule, deleteModule, isDirty, markDirty, resortDependency, isDenpendencySolved } from "./codeload";
...
<font size="2em" color="red">${!isDenpendencySolved() ? "依赖关系未解决" : ""}</font></span>&nbsp&nbsp&nbsp&nbsp
<font size="2em" color="brown">${isDirty() ? "当前的设置需要刷新才能生效" : ""}</font>
<br/>
<hr/>
...
<span id='install_state' style='font-size:1rem;color:#df307f;'></span>`
          ...
          try {
            $.get(str, (dataIn) => {
              try {
                ...
                var st = (typeof data === "string") ? addModule(data) : "接收了一个无效数据值";
                if (typeof st != "string") {
                  onInstall(st);
                } else {
                  onFailure(st);
    ...
    for (var m of Object.entries(all_modules)) {
      ...
      var color = isCore ? "#ff0000" : "#5d2391";
      var ele = `<li id='${meta.id || "impossible"}'><div style='display:inline;'>
<img src='${meta.icon || ""}' width='50' height='50' style="vertical-align:middle;float:left;"></img>
<div style="height: 8em">&nbsp;&nbsp;<span style='font-size:18px;color:${color}'><strong>${meta.name || "Nameless"}</strong>
</span>&nbsp;&nbsp;&nbsp;<span style='font-size:12px;color:#150029;'>${meta.id || "loader.nameless"}@
${meta.version || "1.0.0" + (isCore ? "&nbsp;<span style='color:#ff0000'><b>[COREMOD]</b></span>" : "")}
</span><br/>&nbsp;&nbsp;<span style='font-size:16px;color:#df307f;'>${meta.author || "Someone"}</span>
<br/>&nbsp;&nbsp;<span style='font-size:12px'>${meta.description}</span>
<button style='float:right;' type='button' class='pn pnc remove' onclick='window.notifyUninstall("${meta.id}")'>
<strong>删除模块</strong></button>&nbsp;&nbsp;<button style='float:right;' type='button' class='pn pnc onoff' onclick='window.notifyOnOff("${meta.id}")'>
<strong>${m[1] ? "禁用" : "启用"}</strong></button><button type='button' style='float:right;' class='pn pnc showsrc' mlsource='${meta.id}'>
