...
function addModule(code: string): Map<string, string> | string {
  ...
  if (!isModuleRegex.test($.trim(code))) {
    ...
    setProperty(obj, "id", dataMap.get("id"));
    setProperty(obj, "name", dataMap.get("name") || dataMap.get("id"));
    setProperty(obj, "author", dataMap.get("author") || "Someone");
    setProperty(obj, "icon", dataMap.get("icon") || IMG_MCBBS);
    setProperty(
      obj,
      "description",
      dataMap.get("description") || "No description provided."
    );
    setProperty(obj, "updateURL", dataMap.get("updateURL"));
    setProperty(obj, "apiVersion", dataMap.get("apiVersion"));
    setProperty(obj, "version", dataMap.get("version"));
    if (
      getProperty(obj, "apiVersion") != undefined &&
      getProperty(obj, "apiVersion") != getAPIVersion()
    ) {
