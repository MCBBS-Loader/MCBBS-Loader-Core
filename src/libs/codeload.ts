...
import { setProperty, PermissionManager, Permission, Ticket, UnsupportedOperationException } from "./native";
import { GM, GMDeleteValue, GMGetValue, GMLog, GMSetValue } from "./usfunc";
const STRING_API_VERSION = String(getAPIVersion());
var dirty: boolean = false;
// 权限，该权限使得其可以修改其他模块而不导致被修改模块失效
const PERMISSION_EDIT_SCRIPT:Permission = Permission.forName("loader:edit_script");
// 权限，该权限允许使用GM函数
const PERMISSION_GM_FUNCTION:Permission = Permission.forName("loader:gm_function");
// 权限，该权限允许管理脚本
const PERMISSION_MANAGE_MODS:Permission = Permission.forName("loader:manage_mods");
// 安装一个模块
function addModule(code: string): Map<string, string> | string {
  ...
  if (!isModuleRegex.test(code.trim())) {
    ...
    var depend: string, before: string, after: string, apiVersion: string | undefined, 
      permissions: string | undefined, id: string | undefined = dataMap.get("id");
    if (id === undefined) {
      console.log("No id");
      return "No id";
    }
    var obj: object = {
      ...
      permissions: permissions = dataMap.get("permissions")
    };
    ...
    if (succ) {
      ...
      if(permissions != undefined){
        GMSetValue("permissions-" + id, flitBlank(permissions.split(",")));
      }
      markDirty();
      return dataMap;
    } else {
...
var loadedCode = new Map<HTMLElement, string[]>();
// 加载代码
function mountCode(id: string, code: string): void {
  var node = document.createElement("script");
  node.id = "code-" + id;
  node.innerHTML = `MCBBS.checkedCall((permissionTicket) => {${code}})`;
  loadedCode.set(node, [node.outerHTML, id]);
  // 这里用标准WebAPI添加节点后面的权限检查就能运行，用jQuery添加节点就不能运行，真是见鬼
  document.body.append(node);
  var permissions = GMGetValue("permissions-" + id, []);
  for(var value of permissions) {
    PermissionManager.grantPermission(PermissionManager.moduleTicket(id), Permission.forName(value));
  }
}
// 卸载代码
...
function checkedCall(func: Function) {
  try {
    var console = PermissionManager.consoleTicket();
    GMLog("WTF，你已经有控制台权限了还运行这个函数干嘛？");
    func(console);
  } catch (ex){
    var element = document.currentScript;
    var record = loadedCode.get(element as any);
    if(element && record && record[0] === element.outerHTML){
      func(PermissionManager.moduleTicket(record[1]));
    } else {
      GMLog(`MCBBS Loader拒绝加载${element} - 它似乎是一个想要骗取GM权限的页内脚本`);
    }
  }
}
// 给脚本修改其他脚本
function markScriptDirty(ticket: Ticket, element: HTMLScriptElement) {
  PermissionManager.assertPermission(ticket, PERMISSION_EDIT_SCRIPT);
  var arr = loadedCode.get(element);
  if(arr === undefined) {
    throw new UnsupportedOperationException();
  }
  arr[0] = element.outerHTML;
}
// 加载GM函数
function getGM(ticket: Ticket) {
  PermissionManager.assertPermission(ticket, PERMISSION_GM_FUNCTION);
  return GM;
}
// 操作脚本
function getMods(ticket: Ticket) {
  PermissionManager.assertPermission(ticket, PERMISSION_MANAGE_MODS);
  return {
    addMod: addModule,
    rmMod: (id: string) => {
      deleteModule(id, () => {});
    ...
    setEnabled: (id: string, enabled: boolean) => {
      var mods = GMGetValue("loader.all", {});
      if(!mods[id] || typeof enabled != "boolean")
        throw new UnsupportedOperationException("Mod does not exist");
      mods[id] = enabled;
      GMSetValue("loader.all", mods);
    },
    listMods: () => {
      return Object.keys(GMGetValue("loader.all", {}));
    },
    getCode: (id: string) => {
      return GMGetValue(`code-${id}`, undefined);
    },
    wipeAllData: () => {
      var mods = GMGetValue("loader.all", {});
      for(var id in mods) {
