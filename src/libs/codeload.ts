...
var dirty: boolean = false, dependencySolved: boolean;
function setDenpendencySolved(solved: boolean) {
  dependencySolved = solved;
}
function isDenpendencySolved(): boolean {
...
function resortDependency(): string[] | string {
  ...
  try {
    ...
    GMSetValue("sorted-modules-list", sortedList);
    dependencySolved = true;
    return sortedList;
  } catch (ex) {
    GMSetValue("sorted-modules-list", ex);
    dependencySolved = false;
    return ex;
  }
}
const allowedChars = "1234567890!@#$%^&*()qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM_.", charAllowed: any = {};
